...
function ViewCartItem({ isModal, setIsModal, item }) {
    const [{ OCIT }, dispatch] = useStateValue();
    const [open, setOpen] = useState(false);

    const pathName = window.location.pathname;
    const subPath = pathName.slice(10);

    // const itemData = OCIT.filter((n) => n.id == subPath);
        ...
        <>
            <div className={isModal ? 'hidden' : ''}>
                <div
                    aria-hidden="true"
                    id="defaultModal"
                    className={`flex  overflow-y-auto overflow-x-hidden fixed top-0 right-0 justify-center items-center  z-50 w-full md:inset-0 h-modal md:h-full `}
                >
